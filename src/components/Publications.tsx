  ...
  return (
    <section id="publications" className="scroll-mt-20 bg-white py-16 sm:py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        {/* Filter Buttons */}
        <div className="mt-6 sm:mt-8 flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 px-1">
          {(
            [
            ...
            <button
              ...
              className={`rounded-full px-3 py-1.5 sm:px-4 sm:py-2 text-xs font-bold transition-colors ${
                filter === t.key
        ...
        {filtered.length > 0 ? (
          <div className="mt-8 sm:mt-10 space-y-4 sm:space-y-6">
            {filtered.map((pub) => (
              <div
                key={pub.id}
                className="group rounded-2xl border border-ink-200 bg-white p-5 sm:p-6 shadow-sm transition-all duration-300 hover:border-brand-300 hover:shadow-md"
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="mt-2.5 font-display text-base sm:text-lg font-bold text-ink-900 group-hover:text-brand-600 transition-colors">
                      {pub.title}
                    </h3>

                    <p className="mt-1 text-xs sm:text-sm font-semibold text-ink-700">{pub.authors}</p>
                    <p className="mt-0.5 text-xs italic text-brand-700">{pub.venue}</p>

                    {pub.abstract && (
                      <p className="mt-3 text-xs sm:text-sm leading-relaxed text-ink-600">{pub.abstract}</p>
                    )}
                  </div>

                  <div className="flex flex-wrap shrink-0 items-center gap-2 sm:flex-col sm:items-end w-full sm:w-auto pt-2 sm:pt-0 border-t border-ink-100 sm:border-t-0">
                    {pub.doi && (
                      <a
                        href={pub.doi}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-brand-600 px-3 py-2 text-xs font-bold text-white hover:bg-brand-700 transition-colors flex-1 sm:flex-initial"
                      >
                    ...
                    {pub.bibtex && (
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-ink-200 bg-white px-3 py-2 text-xs font-medium text-ink-700 hover:bg-ink-50 transition-colors flex-1 sm:flex-initial"
